...
export class TorneioService {
  ...
  /**
   * Faz uma requisição http na api e insere os dados do torneio
   * @param nome
   * @param url 
   */
  ...
  /**
   * Faz uma requisição http na pi e atualiza os dados do torneio
   * @param id 
   * @param nome 
   * @param url 
   */
  atualizar(id: number, nome: String, url: String): Promise<any>{
    this.url_api = 'https://api.challonge.com/v1/tournaments.json'+
                                '?api_key=HFQp64rnt3zSP216dLZ1A3KmIXjkyCHf66W4bwAu'+
                                '&tournament[name]='+nome+
                                '&tournament[url]='+url;
